...
class GBNeck_interaction(MessagePassing):
    def __init__(self, parameters, device, unique_radii=None):
        ...
        self.register_buffer(
            "_d0",
            torch.tensor(
                self.createUniqueTable(d0), dtype=torch.float, device=self._device
            ),
        )
        self.register_buffer(
            "_m0",
            torch.tensor(
                self.createUniqueTable(m0), dtype=torch.float, device=self._device
            ),
        )
    ...
    def createUniqueTable(self, fullTable):
        ...
        for i in range(numRadii):
            for j in range(numRadii):
                table.append(
                    weight1[i] * weight1[j] * fullTable[index1[i] * 21 + index1[j]]
    ...
    def message(
        self, x_i: torch.Tensor, x_j: torch.Tensor, edge_attributes: torch.Tensor
    ) -> torch.Tensor:
        ...
        Ineck = torch.where(
            ...
            / (
                1
                + 100 * (r - self.getd0(radindex1, radindex2)) ** 2
                + 0.3 * 1000000 * (r - self.getd0(radindex1, radindex2)) ** 6
            ),
            torch.tensor(0, dtype=torch.float, device=self._device),
        )
...
class GBNeck_energies(MessagePassing):
    def __init__(self, parameters, device, unique_radii=None, solvent_dielectric=78.5):
        """
        GBNeck interaction
        """
        ...
        self.createRadiusToIndexMap()
        self.register_buffer(
            "_d0", torch.tensor(self.createUniqueTable(self.__d0), dtype=torch.float)
        )
        self.register_buffer(
            "_m0", torch.tensor(self.createUniqueTable(self.__m0), dtype=torch.float)
        )
        self._num_unique = len(self._uniqueRadii)

        self.register_buffer(
            "_soluteDielectric", torch.tensor(1, dtype=torch.float, device=self._device)
        )
        self.register_buffer(
            "_solventDielectric",
            torch.tensor(solvent_dielectric, dtype=torch.float, device=self._device),
        )
    ...
    def forward(self, edge_index, x, edge_attributes):
        ...
        pair_energies = self.propagate(
            edge_index, x=x, edge_attributes=edge_attributes, size=None
        )
...
class GBNeck_energies_no_dielectric(GBNeck_energies):

    def forward(self, edge_index, x, edge_attributes, solvent_dielectrics):
        ...
        return (
            -0.5
            * 138.935485
            * (1 / self._soluteDielectric - 1 / solvent_dielectrics)
            * tot_energy
        )
...
class IN_layer_all_swish_2pass(MessagePassing):
    ...
    def __init__(
        self, in_channels, out_channels, cutoff=0.7, device="cuda", hidden=128
    ):
        super().__init__(aggr="add")
        ...
        self.register_buffer(
            "_FREQUENCIES",
            np.pi
            * torch.arange(1, self._NUM_KERNELS + 1, dtype=torch.float, device=device),
        )
...
class IN_layer_all_swish_2pass_tokens(IN_layer_all_swish_2pass):
    ...
    def __init__(
        ...
        device="cuda",
        hidden=128,
        solvent_embedding_size=128,
        dropout_rate=0.0,
        hidden_token=128,
    ):
        super().__init__(in_channels, out_channels, cutoff, device, hidden)

        # Adjust Node MLP
        self.lin1 = torch.nn.Linear(hidden + solvent_embedding_size, hidden_token)
        self.dropout = torch.nn.Dropout(dropout_rate)
        self.lin2 = torch.nn.Linear(hidden_token, out_channels)

    def forward(self, x, edge_index, edge_attributes, solvent_embedding):
        ...
        # Nodewise combination
        # concat_type: (x: torch.Tensor, tokens: torch.Tensor)
        x = torch.concat((x, solvent_embedding), dim=1)
        x = self.lin1(x)
        x = self._silu(x)
        x = self.dropout(x)
        x = self.lin2(x)
...
class IN_layer_all_swish_2pass_tokens_mes_tokens(IN_layer_all_swish_2pass):
    ...
    def __init__(
        ...
        # Adjust Message MLP
        self.message1 = torch.nn.Linear(
            in_channels + self._NUM_KERNELS + solvent_embedding_size, hidden
        )
        self.message2 = torch.nn.Linear(hidden, hidden)
