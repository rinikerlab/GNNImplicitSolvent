...
class Trainer:
        ...
        for i in range(runs):
            ...
            for j, data in enumerate(self._training_data):
                ...
                for d, ldata in enumerate(loader):
                    ...
                    if self._verbose:
                        print(
                            "Iteration %i avg time: %.2f loss: %.2f"
                            % (
                                j,
                                (time.time() - start) / (j + 1),
                                np.mean(loss.to("cpu").tolist()),
                            ),
                            end="\r",
                        )
            ...
            self._model.train()
            print(
                "Run %i avg time: %.3f loss: %.3f"
                % (i, (time.time() - start) / (j + 1), np.sqrt(np.mean(Val_Losses[i]))),
                end="\n",
                flush=True,
            )
            ...
            if wandblogging:
                wandb.log(
                    {
                        "RMSE": np.sqrt(Val_Losses[i]),
                        "time": (time.time() - start) / (j + 1),
                    }
                )

            if make_scheduler_step:
                self._scheduler.step()

            if save_model_after_epochs:
                self.save_model()
    ...
    def validate_per_solvent(
        self, batch_size=16, log_graph=False, solvent_name_dict=None, iteration=0
    ):

        Solvent_validation_loss = []

        pre_forces_list, forces_list, solvent_list = [], [], []
        ...
        for i, data in enumerate(self._validation_data):
            ...
            for l, ldata in enumerate(loader):
                ...
                loss = torch.mean(loss, dim=1).squeeze()

                solvent_loss = torch.bincount(
                    ldata.solvent_model_tensor, weights=loss, minlength=50
                ) / torch.bincount(ldata.solvent_model_tensor, minlength=50)
                Solvent_validation_loss.append(
                    solvent_loss.cpu().detach().numpy()[:, np.newaxis]
                )
                if log_graph:
                    pre_forces_list += pre_forces.flatten().to("cpu").tolist()
                    forces_list += ldata.forces.flatten().to("cpu").tolist()
                    solvent_list += (
                        torch.repeat_interleave(ldata.solvent_model_tensor, 3)
                        .to("cpu")
                        .tolist()
                    )
        ...
        if log_graph:
            start = time.time()
            pre_forces_list = np.array(pre_forces_list)
            forces_list = np.array(forces_list)
            solvent_list = np.array(solvent_list)

            plt.figure(figsize=(10, 10))
            ax = plt.gca()
            self.plot_forces_comparison(
                pre_forces_list, forces_list, hist_bins=100, iax=ax
            )
            wandb.log({"Total chart": wandb.Image(plt), "step": iteration})
            plt.close()

            for i in range(50):
                if len(pre_forces_list[solvent_list == i]) == 0:
                    continue
                plt.figure(figsize=(10, 10))
                ax = plt.gca()
                ax.title.set_text(solvent_name_dict[i])
                self.plot_forces_comparison(
                    pre_forces_list[solvent_list == i],
                    forces_list[solvent_list == i],
                    hist_bins=100,
                    iax=ax,
                    yticks=([1000, 10000], [r"$10^{4}$", r"$10^{5}$"]),
                )
                wandb.log(
                    {
                        "Solvent %s chart" % solvent_name_dict[i]: wandb.Image(plt),
                        "step": iteration,
                    }
                )
                plt.close()

            wandb.log({"graph_time": time.time() - start, "step": iteration})

        solvent_val_arr = np.concatenate(Solvent_validation_loss, axis=1).T

        return np.nanmean(solvent_val_arr, axis=0)
    ...
    @staticmethod
    def fetch_training_data_from_query_pairs(
        query_results, solvent_id=0, solvent_dielectric=78.5
    ):
        """
        ...
        for positions, forces, atomfeatures in query_results:
            ...
            )  # Remove to allow multiprocessing , requires_grad=True)
            force = torch.tensor(forces, dtype=torch.float)
            atom_features = Trainer.reset_gbn2_parameters_for_radii(
            ...
            if (
                (torch.isnan(pos).sum() > 0)
                or (torch.isnan(force).sum() > 0)
                or (torch.isnan(atom_features).sum() > 0)
            ):
                print("Nan in positions or forces or atomfeatures")
                continue
            assert torch.isnan(pos).sum() == 0
            assert torch.isnan(force).sum() == 0
            assert torch.isnan(atom_features).sum() == 0

            solvent_model_id = torch.tensor(
                [solvent_id for i in range(len(atom_features))], dtype=torch.long
            )
            solvent_dielectrics = torch.tensor(
                [solvent_dielectric for i in range(len(atom_features))],
                dtype=torch.float,
            )
            ...
            data = [
                Data(
                    ...
                    solvent_model_tensor=solvent_model_id,
                    water_model_id=torch.tensor(solvent_id, dtype=torch.float),
                    solvent_dielectric=solvent_dielectrics,
                )
            ]
            Dataset += data
    ...
    def create_splitted_data(self, Dataset, train_test_split=0.8):
        """
        Creates the splitted data from a dataset
        """
        training_data, validation_data, test_data = self.do_split(
            Dataset, 1 - train_test_split
        )
    ...
    def get_gbneck2_param_small_molecules(
        ...
        sim = Simulator(work_dir=work_dir, pdb_id=pid, run_name="getparam", rdkit_mol=rdkit_mol)
    ...
    def get_gbneck2_param_small_molecules_unique(
        ...
        sim.forcefield = OpenFF_forcefield_GBNeck2(
            pid, cache=cache, partial_charges=partial_charges
        )
        topology = sim._datahandler.topology
        charges = np.array(
            [
    ...
    def save_model(self, additional_information=""):

        torch.save(
            self._model,
            self._path + "/" + self._name + additional_information + "model.model",
        )
    ...
    def save_dict(self, additional_information=""):
        self._model.eval()
        torch.save(
            self._model.state_dict(),
            self._path + "/" + self._name + additional_information + "model.dict",
        )
    ...
    def plot_train_v_predicted(
        self, return_forces=False, max_s=None, batch_size=20, ax=None
    ):
        print("start")
...
class FilebasedDataset(Dataset):
    ...
    def __init__(self, root_dir, use_tmpdir=False):

        self.root_dir = root_dir
        self._files = os.listdir(root_dir)
    ...
    def __getitem__(self, idx):
        ...
        if (os.environ.get("TMPDIR") is not None) and self._use_tmpdir:
            os.system(
                "cp %s %s"
                % (
                    os.path.join(self.root_dir, self._files[idx]),
                    os.path.join(os.environ.get("TMPDIR"), "tmp.pt"),
                )
            )
            ds = torch.load(os.path.join(os.environ.get("TMPDIR"), "tmp.pt"))
        else:
            ds = torch.load(os.path.join(self.root_dir, self._files[idx]))
